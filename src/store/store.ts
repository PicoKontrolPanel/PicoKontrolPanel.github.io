...
interface AppState {
  ...
  bleReadText: (path: string, onProgress?: (value: number, label: string) => void, signal?: AbortSignal) => Promise<string>;
...
export const useStore = create<AppState>((set, get) => {
  ...
  return {
    ...
    bleReadText: async (path, onProgress, signal) => {
      if (!protocol?.connected) throw new Error('Ingen BLE-forbindelse.');
      return protocol.readText(path, onProgress, signal);
    },
