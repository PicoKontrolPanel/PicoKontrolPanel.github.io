...
export async function installBundledMicroPythonUf2(onProgress?: (value: number, label: string) => void): Promise<string> {
  const picker = (window as WindowWithDirectoryPicker).showDirectoryPicker;
  if (!picker) {
    throw new Error('Browseren kan ikke skrive direkte til Pico-drevet. Brug Chrome eller Edge på computer.');
  }

  onProgress?.(15, 'Henter indbygget UF2...');
  ...
  onProgress?.(35, 'Vælg RPI-RP2-drevet...');
