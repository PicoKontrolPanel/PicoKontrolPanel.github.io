...
`,a=await this.repl.exec(o);if(a.error)throw new Error(a.error);return a.output.split(/\r?\n/).map(d=>d.trim()).filter(Boolean).map(d=>{const[m,p,f]=d.split("	"),g=p==="dir"||p==="file"?p:"unknown",v=parseInt(f??"",10);return{name:m,path:ev(l,m),type:g,...Number.isFinite(v)&&v>=0?{size:v}:{}}})}async readText(l){const o=`
import sys
...
import os
os.rename(${al(l)}, ${al(o)})
`);if(a.error)throw new Error(a.error)}}function nv(i){const l=new Uint8Array(Math.floor(i.length/2));for(let o=0;o<l.length;o+=1)l[o]=parseInt(i.slice(o*2,o*2+2),16);return l}const rv=`from machine import Pin                    # Giver adgang til pins på Pico W, fx LED'en\r
...
`,iv=`from machine import Pin, time_pulse_us\r
...
\r
        ...
        return cms`,av=`import array, time\r
...
while True:\r
    ...
    # til Bluetooth.\r
    time.sleep_ms(100)\r
`,ov=`from machine import Pin                    # Giver adgang til pins på Pico W, fx LED'en\r
import utime as time                       # Bruges til tid, pauser og timere\r
import network                             # Bruges til at forbinde Pico W til Wi-Fi\r
import gc                                  # Bruges til at rydde op i hukommelsen\r
...
while True:
    ...
    if ble.connected:
        ...
        time.sleep_ms(100)
`,pv=`from machine import Pin, PWM, ADC\r
from PicoBluetooth import PicoBluetooth\r
from neopixel import Neopixel\r
